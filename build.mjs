...
// ── nav ─────────────────────────────────────────────────────────────────────
// home=true  → bare #hash links (index.html, already on homepage)
// home=false → /#hash links (all other pages)
...
const MENU_SVG = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>`;

function buildNav(home) {
  const p = home ? '' : '/';
  const logoHref = home ? '#' : '/';
  return `<nav id="navbar" style="position:fixed;top:0;left:0;right:0;z-index:50;background:rgba(250,246,238,.93);backdrop-filter:blur(12px);border-bottom:1px solid transparent;transition:all .3s;">
  <div style="max-width:80rem;margin:0 auto;padding:0 2.5rem;">
    <div style="display:flex;align-items:center;justify-content:space-between;height:4rem;">
      <a href="${logoHref}" style="display:flex;align-items:center;gap:.75rem;text-decoration:none;">
        <div style="width:36px;height:36px;background:#D64000;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          <img src="/common/clinic_assets/dong.svg" alt="冬" style="width:24px;height:24px;filter:brightness(0) invert(1);object-fit:contain;">
        </div>
        <div>
          <img src="/common/clinic_assets/dongsheng.svg" alt="冬勝診所" style="height:28px;width:auto;">
          <div class="label-caps" style="color:#7A7568;font-size:.58rem;">Dong Sheng Clinic</div>
        </div>
      </a>
      <div style="display:none;gap:2rem;align-items:center;" id="desktop-nav">
        <a href="${p}#about" class="nav-link">關於我們</a>
        <a href="${p}#doctor" class="nav-link">醫師介紹</a>
        <a href="${p}#services" class="nav-link">診療項目</a>
        <a href="${p}#pricing" class="nav-link">收費標準</a>
        <a href="${p}#schedule" class="nav-link">看診時間</a>
        <a href="/appointment.html" class="nav-link" style="color:#D64000;font-weight:700;">線上掛號</a>
        <a href="${p}#contact" class="nav-link">聯絡我們</a>
        <a href="/blog.html" class="nav-link">部落格</a>
        <a href="tel:07-727-8392" class="btn-primary" style="padding:.6rem 1.25rem;font-size:.72rem;">
          <span>07-727-8392</span>${PHONE_SVG}
        </a>
      </div>
      <button id="menu-btn" style="padding:.5rem;background:none;border:none;cursor:pointer;color:#1A1710;">${MENU_SVG}</button>
    </div>
  </div>
  <div id="mobile-menu" style="flex-direction:column;background:rgba(250,246,238,.98);border-top:1px solid rgba(26,23,16,.08);padding:1rem 1.5rem 1.5rem;">
    <a href="${p}#services" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">診療項目</a>
    <a href="${p}#about" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">關於我們</a>
    <a href="${p}#doctor" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">醫師介紹</a>
    <a href="${p}#pricing" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">收費標準</a>
    <a href="/blog.html" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">部落格</a>
    <a href="${p}#schedule" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">看診時間</a>
    <a href="${p}#contact" class="nav-link" style="padding:.75rem 0;display:block;border-bottom:1px solid rgba(26,23,16,.06);">聯絡我們</a>
...
// ── inject nav into index.html and appointment.html ─────────────────────────

const NAV_RE = /<!-- BUILD:NAV:START -->[\s\S]*?<!-- BUILD:NAV:END -->/;

function injectNav(filePath, home) {
  let src = fs.readFileSync(filePath, 'utf8');
  if (!NAV_RE.test(src)) {
    console.warn(`⚠ No BUILD:NAV markers found in ${filePath} — skipping`);
    return;
  }
  src = src.replace(NAV_RE, `<!-- BUILD:NAV:START -->\n${buildNav(home)}\n<!-- BUILD:NAV:END -->`);
  fs.writeFileSync(filePath, src, 'utf8');
  console.log(`✓ ${filePath} (nav injected)`);
}

injectNav('src/index.html', true);
injectNav('src/appointment.html', false);

console.log(`\nDone — ${posts.length} posts + nav injected.`);
